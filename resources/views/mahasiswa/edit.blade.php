                    ...
                    <div class="col-md-4 offset-md-4">
                        ...
                        <form action="{{url('mahasiswa/'.$mahasiswa->id)}}" method="POST">
                            ...
                            <div class="form-group">
                                <label>Nama</label>
                                <input type="text" name="Nama" class="form-control" @error('Nama') is-invalid @enderror value="{{old('Nama', $mahasiswa->Nama)}}" autofocus >
                                @error('Nama')
                            ...
                            <div class="form-group">
                                ...
                                <option value="{{$item->id}}" {{old('jurusan_id',$mahasiswa->jurusan_id) == $item->id ? 'selected' : null }}>{{$item->jurusan}}</option>
                                @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label>Email</label>
                                <input type="text" name="email" class="form-control" @error('email') is-invalid @enderror  value="{{old('alamat',$mahasiswa->email)}}">
                                @error('email')
                                <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                            </div>
